...
function App() {
  ...
  // Informações do estudante (se disponíveis)
  const [studentInfo, setStudentInfo] = useState<{name: string; email: string;} | null>(null);

  // Verificar acesso inicial e autenticação
  useEffect(() => {
    // Verificar acesso prévio ao programa Early Access
  ...
  // Handler para o Early Access
  const handleEarlyAccessGranted = () => {
    setHasEarlyAccess(true);
  };

  // Handler para salvar resultados
  const handleSaveResults = (name: string, email: string, course: string, confidence: number) => {
    // Registra o resultado no "banco de dados" local
    addStudentRecord(name, email, course, confidence, answers);
    
    // Salva a informação do estudante no estado
    setStudentInfo({ name, email });
  };

  // Se não tiver acesso, mostrar a página de Early Access
  if (!hasEarlyAccess) {
    return <EarlyAccessGateway onAccessGranted={handleEarlyAccessGranted} />;
  }

  // Determinar qual visão mostrar
  const renderView = () => {
    switch (view) {
      case 'welcome':
