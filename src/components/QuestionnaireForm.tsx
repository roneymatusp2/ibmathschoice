...
import { ArrowLeft, ArrowRight, CheckCircle, BookOpen, BrainCircuit, Calculator, School, GraduationCap, AlertCircle } from 'lucide-react';
...
function QuestionnaireForm({ onSubmit }: QuestionnaireFormProps) {
  ...
  const unansweredSections = sections.map((section, index) => {
    const unansweredQuestions = section.questions.filter(q => !answers[q.id]);
    return {
      index,
      title: section.title,
